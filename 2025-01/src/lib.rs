pub mod part1 {
    ...
    pub fn solution2(file: String) -> u32 {
        ...
        file.lines().for_each(|line| {
            cur_pos += match line {
                l if line.starts_with("L") => -l.split("L").last().unwrap().parse::<i32>().unwrap(),
                l if line.starts_with("R") => l.split("R").last().unwrap().parse::<i32>().unwrap(),
                _ => 0,
            };
    ...
    pub fn solution3(file: String) -> u32 {
        file.lines()
            .fold((50, 0), |(mut curr, mut cnt), line| {
                curr += match line {
                    l if l.starts_with("L") => -l[1..].parse::<i32>().unwrap(),
                    r if r.starts_with("R") => r[1..].parse::<i32>().unwrap(),
                    _ => 0,
                };
                if curr % 100 == 0 {
                    cnt += 1;
...
pub mod part2 {
    ...
    pub fn solution1(file: String) -> u32 {
        file.lines()
            .fold((50, 0), |(mut curr, mut cnt), line| {
                let num = if let Ok(num) = line.strip_prefix("L").unwrap_or("!").parse::<i32>() {
                    -num
