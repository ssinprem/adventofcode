pub mod part1 {
    ...
    pub fn solve(file: String) -> u64 {
        let reg_group = Regex::new(r"mul\((\d+),(\d+)\)").unwrap();
        ...
        reg_group
            ...
            .iter()
            .map(|(_full, [a, b])| {
                a.parse::<u64>().expect("cannot parse") * b.parse::<u64>().expect("cannot parse")
            })
            .sum::<u64>()
    }
}
...
pub mod part2 {
    use regex::Regex;
    pub fn solve(file: String) -> u64 {
        let reg_group = Regex::new(r"(?:mul\((\d+),(\d+)\))|(do\(\))|(don't\(\))").unwrap();

        let mut does = true;
        let mut sum = 0;
        reg_group.captures_iter(file.as_str()).for_each(|capture| {
