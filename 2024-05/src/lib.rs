...
pub fn parse(file: String) -> (HashSet<(u32, u32)>, Vec<Vec<u32>>) {
    let re_pair = Regex::new(r"(\d+)\|(\d+)").expect("Regex invalid pattern failed");

    let re_order = Regex::new(r"((?:\d+)(?:,(?:\d+))+)").expect("Regex invalid pattern failed");
    ...
    pub fn solve(file: String) -> u32 {
        ...
        orders
            ...
            .map(|order| {
                ...
                let middle = *order.get((len - 1) / 2).expect("cannot get middle page");
