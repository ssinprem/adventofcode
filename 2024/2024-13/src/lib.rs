...
pub fn parse(file: String) -> Vec<((i64, i64), (i64, i64), (i64, i64))> {
    let reg = Regex::new(
        r"Button A: X\+(\d+), Y\+(\d+)\nButton B: X\+(\d+), Y\+(\d+)\nPrize: X=(\d+), Y=(\d+)",
    )
    .unwrap();

    reg.captures_iter(file.as_str())
        .map(|cap| {
            (
                (
...
//            A*ay + B*by = ry
pub fn ab_solve(ax: i64, bx: i64, rx: i64, ay: i64, by: i64, ry: i64) -> Option<(i64, i64)> {
    let deter = (ax * by) - (bx * ay);
...
pub mod part1 {
    ...
    pub fn solve(file: String) -> u64 {
        ...
        datas
            .iter()
            .inspect(|d| println!("{d:?}"))
            .filter_map(|((ax, ay), (bx, by), (rx, ry))| ab_solve(*ax, *bx, *rx, *ay, *by, *ry))
            .inspect(|d| println!("{d:?}"))
            .filter_map(|(a, b)| {
