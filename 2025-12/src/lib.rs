...
pub fn parse(
    ...
    HashMap<usize, Vec<Vec<bool>>>,
    Vec<((usize, usize), HashMap<usize, u32>)>,
) {
    let mut map_pattern = HashMap::new();
    let mut vec_yard = Vec::new();
    let re_pattern = Regex::new(r"(?:(\d+):\n((?:[.#]+\n)+))").unwrap();

    for (_full, [id, str]) in re_pattern.captures_iter(file.as_str()).map(|c| c.extract()) {
        map_pattern.insert(
            id.parse().expect("cannot parse id"),
            str.lines()
                .map(|line| {
                    line.chars()
    ...
    let re_yard = Regex::new(r"(?:(\d+)x(\d+)\: ((?:\d+ *)+))").unwrap();
    for (_full, [width, height, amount]) in
        re_yard.captures_iter(file.as_str()).map(|c| c.extract())
    {
        vec_yard.push((
            (
...
pub fn put_yard(
    ...
    for y in 0..size {
        for x in 0..size {
            let cell = pattern
                .get(y)
                .expect("cannot get cell by y {y}")
                .get(x)
                .expect("cannot get pattern by x {x}");
            let target = new_map
                .get_mut(target_y + y)
                .expect("cannot access target y")
                .get_mut(target_x + x)
                .expect("cannot access target x");
            if *cell {
                if *target {
...
pub fn valid_put_yard(
    map: Vec<Vec<bool>>,
    patterns_variants: &HashMap<usize, Vec<Vec<Vec<bool>>>>,
    amounts: Vec<usize>,
) -> bool {
    if amounts.is_empty() {
        println!("{}", display(&map));
    ...
    let size = pattern_variants
        .first()
        .expect("cannot get pattern size")
        .len();
...
pub mod part1 {
    ...
    pub fn solve(file: String) -> u64 {
        ...
        let mut count = 0;
        let patterns_variant: HashMap<usize, Vec<Vec<Vec<bool>>>> = patterns
            .iter()
            .map(|(idx, pat)| (*idx, get_varints(pat)))
            .collect();
        ...
        for (idx, ((width, height), quota)) in yards.iter().enumerate() {
            ...
            let pattern_set = quota
                .iter()
                .flat_map(|(&id, &amount)| (0..amount).map(|_| id).collect::<Vec<usize>>())
                .collect::<Vec<usize>>();
            // calculate all part area
            let part_area = pattern_set
                .iter()
                .map(|idx| {
                    patterns
                        .get(idx)
                        .expect("Cannot get pattarn by id")
                        .iter()
