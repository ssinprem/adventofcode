...
pub mod part1 {
    ...
    pub fn solve(file: String, cnt: u64) -> u64 {
        ...
        for entry in rank.iter().take_while(|(_, dist)| *dist <= threshold_dist) {
            ...
            if circuits
                .iter()
                .any(|c| c.contains(&key.0) || c.contains(&key.1))
            {
                let exists = circuits
                    .iter()
                    .filter(|c| c.contains(&key.0) || c.contains(&key.1))
                    .collect::<Vec<&HashSet<JBox>>>();
                let mut new_circuit = exists.iter().fold(HashSet::new(), |mut new, circuit| {
                ...
                circuits = circuits
                    .iter()
                    .filter(|target| !exists.contains(target))
                    .cloned()
                    .collect::<Vec<HashSet<JBox>>>();
