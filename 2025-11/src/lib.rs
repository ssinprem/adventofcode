pub fn parse(file: String) -> Vec<(String, String)> {
    file.lines().fold(vec![], |mut vec, line| {
        if let Some((src, dsts)) = line.split_once(":") {
            for dst in dsts.split(" ").filter(|d| !d.is_empty()) {
...
pub mod part1 {
    ...
    pub fn solve(file: String) -> u64 {
        ...
        if let Some(paths) = find_path(&graph, &"you".to_string(), &"out".to_string(), vec![]) {
            paths.len() as u64
        } else {
            unreachable!()
        }
...
pub mod part2 {
    ...
    pub fn dfs(
        ...
        let count = graph
            ...
            .map(|(_src, dst)| dfs(dst.to_string(), has_dac, has_fft, graph, memo))
            .sum();
